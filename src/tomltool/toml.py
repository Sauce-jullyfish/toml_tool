...
class TomlTool:
    def __init__(self, data, filename=None):
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                setattr(self, key, TomlTool(value))
            else:
    ...
    def load(cls, filename: str):
        """
        Load TOML data from the specified file and create a new instance of TomlTool.

        Args:
            filename (str): The path of the TOML file to load.

        Returns:
            TomlTool: An instance of TomlTool containing the data from the file.

        Example:
            toml = TomlTool.load('file.toml')
        """
        ...
        for idx, k in enumerate(keys):
            if not hasattr(current_level, k):
                if idx == len(keys) - 1:
                    ...
                    setattr(current_level, k, TomlTool({}))
            else:
                if idx == len(keys) - 1:
                    setattr(current_level, k, value)
    ...
    def to_dict(self):
        ...
        for key, value in self.__dict__.items():
            if key == '__file__' or value is None:
                continue
            elif isinstance(value, TomlTool):
